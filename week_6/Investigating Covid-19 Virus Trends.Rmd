...
write.csv(boot_tbl, "week6_bootstrap_summary.csv", row.names = FALSE)
...
write.csv(jk_df, "week6_jackknife_coefs.csv", row.names = FALSE)
...
p_jk <- ggplot(jk_df, aes(coef)) +
  ...
  theme_minimal()
ggsave("week6_jackknife_hist.pdf", p_jk, width = 7, height = 4.5, dpi = 300)
...
p_ci <- ggplot(ci_df, aes(Date, g_mean)) +
  ...
  theme_minimal()
ggsave("week6_ci_band.pdf", p_ci, width = 8, height = 4.8, dpi = 300)
```
